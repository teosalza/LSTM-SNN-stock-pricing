...
from enc_dec_lstm import ENC_DEC_LSTM
...
if __name__ == "__main__":
    ...
    # model_weight_path = WEIGHT_DIR + "/"+"enc-dec-lstm_2.pt"
    ...
    model_enc_dec_lstm = ENC_DEC_LSTM(
    ...
    model_enc_dec_lstm.load_state_dict(torch.load(model_weight_path))
    model_enc_dec_lstm.eval()
    ...
    for ii, (data,target)  in enumerate(train_loader_test):
        ...
        data = data.to(model_enc_dec_lstm.device)
        target = target.to(model_enc_dec_lstm.device) 

        pred = model_enc_dec_lstm.forward(data)

        linear_loss = model_enc_dec_lstm.criterion(pred,target)
