...
if __name__ == "__main__":
    ...
    # model_weight_path = WEIGHT_DIR + "/"+"ann_2.pt"
    ...
    model_ann.load_state_dict(torch.load(model_weight_path,map_location="cpu"))
    model_ann.eval()
    ...
    for ii, (data,target)  in enumerate(train_loader_test):
        ...
        data = data.to(model_ann.device)
        target = target.to(model_ann.device) 

        pred = model_ann.forward(data)

        linear_loss = model_ann.criterion(pred,target)
        ...
        for i in range(max_range):
            if i == 0:
                if ii > 0:
                    if pred[0][i].item()-old_pred[0][0].item() > 0:
                        pred_trend.append(1)
                    else:
                        pred_trend.append(0)

                    if target[0][i].item()-old_actual[0][0].item() > 0:
    ...
    y_pred_transformed = scaler.inverse_transform(np.concatenate([dataset.iloc[-x_test.shape[0]:,:-1],np.array(y_pred)],axis=1))[:,-1:]
    y_actual_transformed = scaler.inverse_transform(np.concatenate([dataset.iloc[-x_test.shape[0]:,:-1],np.array(y_actual)],axis=1))[:,-1:]
